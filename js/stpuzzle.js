...
var emptytilePosRow = getPosBlank()[0];
var emptytilePosCol = getPosBlank()[1];
var cellDisplacement = "70px";
		...
		function moveTile() 
		{
			...
			if(compareArray(getArray(),[[1,2,3],[4,5,6],[7,8,0]])) win();
		...
		function getArray(){
			...
			for(var i=0; i < 3; i++){
				for(var j=0; j < 3; j++){
					//console.log(counter)
					pos[i][j] = parseInt(getByCoords([i,j]).dataset.num);
				}
			}
			return pos;
		}
		...
		function getByCoords(pos){
			var y = pos[0];
			var x = pos[1]
			var cells = document.querySelectorAll("[data-pos]")
			for(var i = 0; i < cells.length; i++){
				if(cells[i].dataset.pos == y+','+x) return cells[i];
			}
		}

		function getPosBlank(){
			var pos = document.querySelector("#empty").dataset.pos.split(",")
			pos[0] = parseInt(pos[0]);
			pos[1] = parseInt(pos[1]);
			return pos
		}

		function getRadio(pos){
			var y = pos[0];
			var x = pos[1];
			var radio = [];
			radio['a'] = null;
		...
		function start() {
			...
			var init = new Node(0, getArray(), getPosBlank()[0], getPosBlank()[1], 0)
			...
			//console.log('Solution: ' + result.path + ' Total steps: ' + result.path.length);
			
			//getByCoords([0,0]).click();
			//try {
		...
		function solve(result, i){
			...
			setTimeout(function(){
				var NextAction = re[0];
				var BlankPos = getPosBlank();
				var BlankRadio = getRadio(getPosBlank());
				var ClickElement = BlankRadio[NextAction];

				/*console.log(NextAction);
				console.log(BlankPos);
				console.log(BlankRadio)
				console.log(ClickElement)*/

				getByCoords(ClickElement).click();
				
				solve(re.substr(1),++counter)
			},(1000+(counter*2)) )
		}
